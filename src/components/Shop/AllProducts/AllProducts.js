...
function AllProducts() {
  ...
  const { onAddToCart, onAddToFavorite, searchItems, isLoading, setIsLoading } =
  ...
  const filterProduct = products.filter((card) => {
    return card.title.toLowerCase().includes(searchItems.toLowerCase());
  });
  ...
  return (
    <div className="shop">
      ...
      <ul className="shop__list">
        {filterProduct.slice(0, visible).map((product) => (
          <li key={product._id}>
            <div className="trand-card__container">
              <Link className="shop__link" to={`/all/${product._id}`}>
                <img

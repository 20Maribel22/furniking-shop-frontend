...
function SinglePageWardrobe() {
  const { id } = useParams();
  const navigate = useNavigate();
  const [product, setProduct] = useState({});

  const { onAddToCart, onAddToFavorite, isLoading, setIsLoading } = useContext(AppContext);
                ...
                {product.countInStock > 0 ? (
                  <>
                    <li className="product-single__text">
                       <p><b>Quantity:</b>&nbsp;{product.countInStock}</p>
                    </li>
...
export default SinglePageWardrobe;
